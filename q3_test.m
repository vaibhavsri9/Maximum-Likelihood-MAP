...
performance_array = zeros(6,2000);
for B = 1:10 
    x_train = datasample(X_1000,2000,2);
    x_valid = datasample(X_1000,2000,2);
    for m = 1:6
...
[maxval,model_selected] = max(sum(performance_array,2)./2000);
...
bar(X,Y)
xlabel('Model Order')
ylabel('No. of times model order got selected')